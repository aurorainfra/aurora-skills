...
macOS. Harness: **Claude Code**. Production. No Aurora account, no API key, never visited the site.
You have no browser. If a step needs a human at one, halt and say exactly what they must do.
...
2. ****The proxy** — if a translating proxy is required, say what you were told to install, where its config goes, and how to start it. If the instructions did not say, say that.**
