...
macOS. Harness: **OpenCode**. Production. **You are already fully set up and it works** — a key in
`.env` here and a working Aurora provider in your OpenCode config. You forgot, and pasted the setup
line again a month later. Treat `.env` as opaque. What you want: not to be walked through signup
again, and above all not to have your working config broken or duplicated.
...
2. ****Detection and idempotency** — did the instructions make you detect the existing setup before acting, or would you have started from signup? Was that check instructed or invented? Hash the config and `.env` (hashes only) before and after: did anything change or duplicate?**
